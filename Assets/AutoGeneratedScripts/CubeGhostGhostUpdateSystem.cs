...
public class CubeGhostGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<CubeGhostSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<MovableGhost> ghostMovableGhostType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostMovableGhostArray = chunk.GetNativeArray(ghostMovableGhostType);
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
                ...
                ghostMovableGhostArray[entityIndex] = ghostMovableGhost;
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<CubeGhostSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<MovableGhost>(),
        ...
        {
            All = new []{
                ComponentType.ReadOnly<CubeGhostSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
        ...
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<CubeGhostSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<CubeGhostSnapshotData>(true),
                ...
                ghostMovableGhostType = GetArchetypeChunkComponentType<MovableGhost>(),
...
public partial class CubeGhostGhostSpawnSystem : DefaultGhostSpawnSystem<CubeGhostSnapshotData>
{
    struct SetPredictedDefault : IJobParallelFor
    {
        [ReadOnly] public NativeArray<CubeGhostSnapshotData> snapshots;
        ...
        {
            if (localPlayerId.Length == 1 && snapshots[index].GetMovableGhostPlayerId() == localPlayerId[0].Value)
                predictionMask[index] = 1;
        }
    }
    protected override JobHandle SetPredictedGhostDefaults(NativeArray<CubeGhostSnapshotData> snapshots, NativeArray<int> predictionMask, JobHandle inputDeps)
    {
