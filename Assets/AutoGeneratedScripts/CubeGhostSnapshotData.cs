...
public struct CubeGhostSnapshotData : ISnapshotData<CubeGhostSnapshotData>
{
    public uint tick;
    private int MovableGhostPlayerId;
    ...
    public int GetMovableGhostPlayerId(GhostDeserializerState deserializerState)
    {
        return (int)MovableGhostPlayerId;
    }
    public int GetMovableGhostPlayerId()
    {
        return (int)MovableGhostPlayerId;
    }
    public void SetMovableGhostPlayerId(int val, GhostSerializerState serializerState)
    {
        MovableGhostPlayerId = (int)val;
    }
    public void SetMovableGhostPlayerId(int val)
    {
        MovableGhostPlayerId = (int)val;
    }
    ...
    public void PredictDelta(uint tick, ref CubeGhostSnapshotData baseline1, ref CubeGhostSnapshotData baseline2)
    {
        var predictor = new GhostDeltaPredictor(tick, this.tick, baseline1.tick, baseline2.tick);
        MovableGhostPlayerId = predictor.PredictInt(MovableGhostPlayerId, baseline1.MovableGhostPlayerId, baseline2.MovableGhostPlayerId);
    ...
    public void Serialize(int networkId, ref CubeGhostSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (MovableGhostPlayerId != baseline.MovableGhostPlayerId) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(MovableGhostPlayerId, baseline.MovableGhostPlayerId, compressionModel);
    ...
    public void Deserialize(uint tick, ref CubeGhostSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            MovableGhostPlayerId = reader.ReadPackedIntDelta(baseline.MovableGhostPlayerId, compressionModel);
        else
            MovableGhostPlayerId = baseline.MovableGhostPlayerId;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref CubeGhostSnapshotData target, float factor)
    {
